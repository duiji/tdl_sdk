...
CVI_S32 CVI_TDL_Detection_Windows(const cvitdl_handle_t handle, VIDEO_FRAME_INFO_S *frame,
  ...
  if (model->isInitialized()) {
    if (initVPSSIfNeeded(ctx, model_index) != CVI_SUCCESS) {
      ...
      CVI_S32 ret = model->windows_inference(frame, obj);
      if (ret != CVI_TDL_SUCCESS)
        return ret;
      else
        return model->after_inference();
 ...
 */
/*
DLL_EXPORT CVI_S32 CVI_TDL_Zbar_Decode(const cvitdl_handle_t handle, VIDEO_FRAME_INFO_S *frame,
                                     CVI_TDL_SUPPORTED_MODEL_E model_index, cvtdl_object_info_t *info, 
                                     char *zbarInfo){
  ...
  if (model->isInitialized()) {
    if (initVPSSIfNeeded(ctx, model_index) != CVI_SUCCESS) {
      ...
      CVI_S32 ret = model->zbarDecode(frame, info, zbarInfo);
      if (ret != CVI_TDL_SUCCESS)
        return ret;
      else
        return model->after_inference();

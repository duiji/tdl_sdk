...
class DetectionBase : public Core {
 public:
  ...
  virtual int windows_inference(VIDEO_FRAME_INFO_S *srcFrame, cvtdl_object_t *obj_meta) {
    LOGE("windows_inference function not implement!\n");
    return 0;
  }

  /*
  virtual int zbarDecode(VIDEO_FRAME_INFO_S *srcFrame, cvtdl_object_info_t *info, char *zbarInfo) {
    LOGE("zbarDecode function not implement!\n");
    return 0;
  }
  */

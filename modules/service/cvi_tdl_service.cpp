...
CVI_S32 CVI_TDL_Service_DrawPoseKeypoint(cvitdl_service_handle_t handle, VIDEO_FRAME_INFO_S *frame,
                                         const cvtdl_object_t *obj_meta) {
  if (obj_meta->size <= 0) return CVI_TDL_SUCCESS;

  if (handle != NULL) {
    return cvitdl::service::DrawPoseKeypoint(obj_meta, frame);
  }
  LOGE("service handle is NULL\n");
  return CVI_TDL_FAILURE;
}

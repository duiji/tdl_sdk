...
int DrawPoseKeypoint(const cvtdl_object_t *obj_meta, VIDEO_FRAME_INFO_S *bg) {
#ifdef NO_OPENCV
  LOGW("no opencv could not draw pose");
  return CVI_TDL_FAILURE;
#else
  ...
  for (int i = 0; i < obj_meta->size; i++) {
    ...
    for (int k = 0; k < 19; k++) {
      ...
      if (obj_meta->info[i].pedestrian_properity->pose_17.score[kps1] < 0.25 ||
          obj_meta->info[i].pedestrian_properity->pose_17.score[kps2] < 0.25)
        continue;
